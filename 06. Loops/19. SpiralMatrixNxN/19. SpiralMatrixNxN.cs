...
class SpiralMatrixNxN
{
    static void Main()
    {
        ...
        string formatNum = numN * numN + ""; // formating string lenght

        for (int row = 0; row < numN; row++) // print result
        {
            for (int col = 0; col < numN; col++)
            {
                string formatOut = Convert.ToString(matrixN[row, col]).PadRight(formatNum.Length + 1, ' ');
                Console.Write(formatOut);
            }
            Console.WriteLine();
        }
    }
